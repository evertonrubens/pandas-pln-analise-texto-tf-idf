    ...
    for token in tokens:
        if token in negations:
            ...
            if token in ["interface", "produtividade", "funcionalidades", "gestão", "integrações", "usar"]:
                if token == "interface":
                    new_tokens.append("não tem uma boa interface")
                elif token == "produtividade":
                    new_tokens.append("não tem produtividade")
                    ...
                    new_tokens.append("é difícil as integrações")
                elif token == "usar":
                    new_tokens.append("é difícil usar")
                elif token == "boa":
                    new_tokens.append("é péssimo") 
                elif token == "desempenho":
                    new_tokens.append("não desempenha")    
                elif token == "tem":
                    new_tokens.append("não tem")     
                elif token == "usar":
                    new_tokens.append("não usar")   
                elif token == "segurança":
                    new_tokens.append("insegurança")             
            else:
...
# Salvar DataFrames como arquivos CSV
